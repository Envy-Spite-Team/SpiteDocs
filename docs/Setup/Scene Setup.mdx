...
This is `StatsManager`. It defines ranks and secrets for the level. The ranks are defined in an ascending order, from the top to bottom(D at the top and S at the bottom). In the `Secret Objects` tab, secret orbs(bonuses) are defined. This works only with bonus objects.
